...
export function requireProjectPermission(permission: string): RequestHandler {
  return async (req, _res, next) => {
    ...
    if (!access.proprietario && !access.permissoes.has(permission)) {
...
export const requireSystemAdmin: RequestHandler = async (req, _res, next) => {
  if (!req.usuarioId) throw new AppError(401, 'Autenticação necessária.', 'NAO_AUTENTICADO')
  const { rowCount } = await query('SELECT 1 FROM usuarios WHERE id=$1 AND ativo AND administrador_sistema', [req.usuarioId])
  if (!rowCount) throw new AppError(403, 'Área restrita à administração do sistema.', 'ACESSO_NEGADO')
  next()
}
