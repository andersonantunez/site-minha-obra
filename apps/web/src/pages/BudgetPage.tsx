...
type CashFlowItem = { id: string; origem: 'MANUAL' | 'PAGAMENTO'; origem_id: number; data: string; descricao: string; detalhes: string | null; quantidade: string | null; unidade: string | null; valor: string; editavel: boolean; provisionado: boolean }
...
function CashFlowRow({ projectId, item, expanded, onExpand, canEdit, canDelete, onEdit, onRemove }: { projectId?: string; item: CashFlowItem; expanded: boolean; onExpand: () => void; canEdit: boolean; canDelete: boolean; onEdit: () => void; onRemove: () => void }) {
  ...
  const quantity=item.quantidade?`${new Intl.NumberFormat('pt-BR',{maximumFractionDigits:3}).format(Number(item.quantidade))} ${item.unidade||''}`.trim():''
  return <><tr className="cash-flow-row" onClick={onExpand}><td>{formatDate(item.data)}</td><td><strong>{item.descricao}</strong>{item.origem === 'PAGAMENTO' && <small>{quantity&&`${quantity} · `}Pagamento sincronizado</small>}</td><td className={`money-column ${value >= 0 ? 'cash-value-positive' : 'cash-value-negative'}`}>{value > 0 ? '+' : ''}{formatMoney(value)}</td><td><span className={`provision-badge ${item.provisionado ? 'yes' : 'no'}`}>{item.provisionado ? 'Sim' : 'Não'}</span></td><td className="row-actions">{item.editavel ? <>{canEdit && <button title="Editar lançamento" onClick={event=>{event.stopPropagation();onEdit()}}><Pencil /></button>}{canDelete && <button className="danger" title="Excluir lançamento" onClick={event=>{event.stopPropagation();onRemove()}}><Trash2 /></button>}</> : <small className="locked-source">Automático</small>}</td></tr>{expanded && <tr className="cash-flow-details"><td colSpan={5}><strong>{item.origem === 'PAGAMENTO' ? 'Detalhes do pagamento' : 'Detalhes do lançamento'}</strong><p>{item.detalhes || 'Nenhum detalhe informado.'}</p>{item.origem==='PAGAMENTO'&&projectId&&<CashFlowPaymentDocuments projectId={projectId} paymentId={item.origem_id}/>}</td></tr>}</>
}
