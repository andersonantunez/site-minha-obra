...
import { formatStageOption, type StageOption } from '../lib/stage'
import { useProjectAccess } from '../lib/projectAccess'

type ProductLink={id:number;url:string;loja:string}
type Payment={id:number;descricao:string;fornecedor:string|null;contato_fornecedor:string|null;nome_contato_fornecedor:string|null;observacao:string|null;ordem:number;etapa_id:number|null;etapa:string|null;quantidade:string|null;unidade:string|null;chave_pix:string|null;valor:string|null;status:PaymentStatus;forma_pagamento:string|null;data_pagamento:string|null;data_agendamento:string|null;data_entrega:string|null;links_cotacao:ProductLink[];quantidade_documentos:number}
type PaymentList={pagamentos:Payment[];total:number}
type Stage=StageOption
type PaymentDocument={id:number;titulo:string;categoria:string;url:string|null;criado_em:string}
...
const paymentStageColor=(payment:Payment)=>(payment as Payment & {etapa_cor?:string|null}).etapa_cor||undefined

function PaymentDocuments({projectId,payment,editable=false,compact=false}:{projectId:string;payment:Payment;editable?:boolean;compact?:boolean}){
  const client=useQueryClient()
  const[source,setSource]=useState<'ARQUIVO'|'LINK'>('LINK')
  const endpoint=`/projetos/${projectId}/pagamentos/${payment.id}/documentos`
  const{data,error}=useQuery({queryKey:['documentos-pagamento',projectId,payment.id],queryFn:()=>api<{documentos:PaymentDocument[]}>(endpoint)})
  const save=useMutation({mutationFn:(form:FormData)=>api(endpoint,{method:'POST',body:form}),onSuccess:()=>{void client.invalidateQueries({queryKey:['documentos-pagamento',projectId,payment.id]});void client.invalidateQueries({queryKey:['pagamentos',projectId]})}})
  const remove=useMutation({mutationFn:(id:number)=>api(`${endpoint}/${id}`,{method:'DELETE'}),onSuccess:()=>{void client.invalidateQueries({queryKey:['documentos-pagamento',projectId,payment.id]});void client.invalidateQueries({queryKey:['pagamentos',projectId]})}})
  ...
  return <section className="payment-resource-panel">
    <header><div><Paperclip/><strong>Documentos relacionados</strong><span>{data?.documentos.length||0}</span></div></header>
    {error&&<ErrorNotice message={error.message}/>}<div className="payment-document-list">{data?.documentos.map(document=><div key={document.id}><FileText/><p><strong>{document.titulo}</strong><small>{document.categoria} · {formatDate(document.criado_em)}</small></p><a href={document.url||`/api${endpoint}/${document.id}/arquivo`} target="_blank" rel="noopener noreferrer"><ExternalLink/>Abrir</a>{editable&&<button className="danger" type="button" onClick={()=>confirm('Excluir este documento?')&&remove.mutate(document.id)}><Trash2/></button>}</div>)}</div>
    {editable&&<form className="resource-inline-form" onSubmit={event=>{event.preventDefault();const target=event.currentTarget;save.mutate(new FormData(target),{onSuccess:()=>target.reset()})}}><input name="titulo" required placeholder="Título do documento"/><input type="hidden" name="categoria" value="Pagamentos"/><select value={source} onChange={event=>setSource(event.target.value as 'ARQUIVO'|'LINK')}><option value="LINK">Link</option><option value="ARQUIVO">Arquivo</option></select>{source==='LINK'?<input name="url" type="url" required placeholder="https://…"/>:<input name="arquivo" type="file" required accept="application/pdf,image/jpeg,image/png,image/webp"/>}<button className="app-button" disabled={save.isPending}><Plus/>Adicionar</button></form>}
  </section>
}

function PaymentLinks({projectId,payment,editable=false,compact=false}:{projectId:string;payment:Payment;editable?:boolean;compact?:boolean}){
  const client=useQueryClient()
  const[draft,setDraft]=useState('')
  const[links,setLinks]=useState(payment.links_cotacao)
  const endpoint=`/projetos/${projectId}/pagamentos/${payment.id}/links`
  const add=useMutation({mutationFn:(url:string)=>api<{link:ProductLink}>(endpoint,{method:'POST',...jsonBody({url})}),onSuccess:result=>{setDraft('');setLinks(current=>current.some(link=>link.id===result.link.id)?current:[...current,result.link]);void client.invalidateQueries({queryKey:['pagamentos',projectId]})}})
  const remove=useMutation({mutationFn:(id:number)=>api(`${endpoint}/${id}`,{method:'DELETE'}),onSuccess:(_,id)=>{setLinks(current=>current.filter(link=>link.id!==id));void client.invalidateQueries({queryKey:['pagamentos',projectId]})}})
  if(compact)return <section className="cash-flow-payment-documents"><strong>Links de cotação</strong>{!links.length?<small>Nenhum link de cotação vinculado.</small>:<div>{links.map(link=><a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer"><Link2/><span>{link.loja} · {link.url}</span><ExternalLink/></a>)}</div>}</section>
  return <section className="payment-resource-panel">
    <header><div><Link2/><strong>Links de cotação</strong><span>{links.length}</span></div></header>
    <div className="quote-link-list">{links.map(link=><div key={link.id}><Link2/><strong>{link.loja}</strong><a className="quote-url" href={link.url} target="_blank" rel="noopener noreferrer">{link.url}</a>{editable&&<button type="button" onClick={()=>void navigator.clipboard.writeText(link.url)}><Copy/></button>}<a href={link.url} target="_blank" rel="noopener noreferrer"><ExternalLink/></a>{editable&&<button className="danger" type="button" onClick={()=>remove.mutate(link.id)}><Trash2/></button>}</div>)}</div>
    {editable&&<form className="resource-inline-form" onSubmit={event=>{event.preventDefault();if(draft)add.mutate(draft)}}><input value={draft} onChange={event=>setDraft(event.target.value)} type="url" placeholder="https://fornecedor.com/cotacao"/><button className="app-button" disabled={!draft||add.isPending}><Plus/>Adicionar</button></form>}
  </section>
}
...
export function PaymentsPage(){
  ...
  const paymentSort=useSortableData(data?.pagamentos||[],'data_pagamento','desc','first')
  const save=useMutation({mutationFn:({id,payload}:{id?:number;payload:Record<string,unknown>})=>api(`${endpoint}${id?`/${id}`:''}`,{method:id?'PUT':'POST',...jsonBody(payload)}),onSuccess:()=>{setEditing(null);void client.invalidateQueries({queryKey:['pagamentos',projetoId]});void client.invalidateQueries({queryKey:['dashboard',projetoId]})}})
  ...
  const submit=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const resources=editing==='new'?{links_cotacao:String(form.get('links_cotacao')||'').split('|').map(value=>value.trim()).filter(Boolean),documentos:String(form.get('documentos')||'').split('|').map(value=>value.trim()).filter(Boolean)}:{};save.mutate({id:editing==='new'?undefined:editing?.id,payload:{etapa_id:form.get('etapa_id')||null,quantidade:form.get('quantidade')||null,unidade:form.get('unidade')||null,descricao:form.get('descricao'),fornecedor:form.get('fornecedor')||null,contato_fornecedor:form.get('contato_fornecedor')||null,nome_contato_fornecedor:form.get('nome_contato_fornecedor')||null,chave_pix:form.get('chave_pix')||null,valor:form.get('valor')||null,status:form.get('status'),forma_pagamento:form.get('forma_pagamento')||null,data_pagamento:form.get('data_pagamento')||null,data_agendamento:form.get('data_agendamento')||null,data_entrega:form.get('data_entrega')||null,observacao:form.get('observacao')||null,ordem:form.get('ordem')||0,...resources}})}
  ...
  return <div>
    ...
    {editing&&<div className="dialog-backdrop"><section className="dialog large payment-dialog"><header><div><small>PAGAMENTO</small><h2>{editing==='new'?'Novo pagamento':'Editar pagamento'}</h2></div><button onClick={()=>setEditing(null)}><X/></button></header><form onSubmit={submit}>
      ...
      <label className="field"><span>Etapa</span><select name="etapa_id" defaultValue={editing==='new'?'':editing.etapa_id||''}><option value="">Sem etapa</option>{stages?.etapas.map(stage=><option value={stage.id} key={stage.id}>{formatStageOption(stage,stages?.etapas||[])}</option>)}</select></label>
      ...
      {editing==='new'&&<><label className="field span-2"><span>Documentos relacionados <small>(links separados por |)</small></span><textarea name="documentos" rows={2}/></label><label className="field span-2"><span>Links de cotação <small>(separados por |)</small></span><textarea name="links_cotacao" rows={2}/></label></>}
      <footer className="form-actions span-2">{editing!=='new'&&<button type="button" className="app-button danger-action" onClick={()=>confirm('Excluir este pagamento?')&&remove.mutate(editing.id)}><Trash2/>Excluir</button>}<button type="button" className="app-button" onClick={()=>setEditing(null)}>Cancelar</button><button className="app-button primary" disabled={save.isPending}>{save.isPending?'Salvando…':'Salvar pagamento'}</button></footer>
    </form>{editing!=='new'&&projetoId&&<div className="editor-resources"><PaymentDocuments projectId={projetoId} payment={editing} editable/><PaymentLinks projectId={projetoId} payment={editing} editable/></div>}</section></div>}
