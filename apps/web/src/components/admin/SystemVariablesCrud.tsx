...
import { AddButton, EmptyState, ErrorNotice, SortableHeader } from '../Ui'
...
export function SystemVariablesCrud() {
  ...
  const variableSort = useSortableData(variables.data?.variaveis || [], 'chave')
  ...
  return <section className="system-variables-crud">
    ...
    <section className="data-table-wrap system-variables-table">
      ...
      {variables.data?.variaveis.length ? <table className="data-table"><thead><tr><SortableHeader<SystemVariable> label="Chave" column="chave" activeColumn={variableSort.sort.key} direction={variableSort.sort.direction} onSort={variableSort.toggle}/><SortableHeader<SystemVariable> label="Valor" column="valor" activeColumn={variableSort.sort.key} direction={variableSort.sort.direction} onSort={variableSort.toggle}/><SortableHeader<SystemVariable> label="Descrição" column="descricao" activeColumn={variableSort.sort.key} direction={variableSort.sort.direction} onSort={variableSort.toggle}/><SortableHeader<SystemVariable> label="Atualização" column="atualizado_em" activeColumn={variableSort.sort.key} direction={variableSort.sort.direction} onSort={variableSort.toggle}/><th>Ações</th></tr></thead><tbody>{variableSort.sorted.map(variable => <tr key={variable.id}><td><strong>{variable.chave}</strong></td><td><pre className="system-variable-value">{formatJson(variable.valor)}</pre></td><td>{variable.descricao}</td><td>{formatDate(variable.atualizado_em)}</td><td><div className="row-actions"><button aria-label={`Editar ${variable.chave}`} onClick={() => { setJsonError(''); setEditing(variable) }}><Pencil /></button><button className="danger" aria-label={`Excluir ${variable.chave}`} onClick={() => confirm(`Excluir a variável ${variable.chave}?`) && remove.mutate(variable.id)}><Trash2 /></button></div></td></tr>)}</tbody></table> : <EmptyState title="Nenhuma variável cadastrada" description="Cadastre configurações globais tipadas para a plataforma." />}
    </section>
