...
const preferredColumns = ['data_pagamento', 'etapa_id', 'quantidade', 'unidade', 'descricao', 'fornecedor', 'valor', 'status', 'data_agendamento', 'data_entrega', 'forma_pagamento', 'observacao']
const cashFlowColumns = ['data', 'descricao', 'detalhes', 'valor']
const scheduleColumns=['tipo','etapa_pai','ordem','nome','descricao','data_inicio_previsto','data_fim_previsto','valor_previsto','data_inicio','data_fim']
...
export function ImportDialog({ endpoint, entityName, onClose, onComplete }: Props) {
  ...
  const sampleBase = isCashFlow ? 'fluxo-caixa' : normalizedEntity.includes('pagamento') ? 'pagamentos' : 'cronograma'
